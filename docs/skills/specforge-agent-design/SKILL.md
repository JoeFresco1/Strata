# SpecForge Agent Design Skill

## When To Use

Use this skill when working on any of the following in SpecForge:
...
Use it when the question is not just "write code," but "how should SpecForge think, coordinate, and move through layered generation work?"

## Project Intent

SpecForge is not a general chatbot. It is a local recursive product-spec generator with human-in-the-loop pruning.
...
- local GGUF models
- Python service layer in `specforge/`
...
SpecForge should widen or deepen a layer aggressively, but only inside bounded passes.
...
Layer 2 should optimize for broad subfeature discovery within approved pillars.

Layer 3 should optimize for implementation-ready detail while minimizing overlap with shared capabilities already found elsewhere.
...
## Recommended Agent Shape For SpecForge
...
Use when multiple branches start producing the same subfeature family.
...
## Recommended Commands For A SpecForge Agent
...
Do not treat SpecForge as a classic document-RAG product by default.
...
A good SpecForge agent workflow should feel like:
...
For SpecForge, the default orchestration stance should be:
