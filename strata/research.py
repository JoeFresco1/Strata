...
from strata.layer2_research import Layer2ResearchMixin
from strata.models import Node, PillarResearchAssessment, PillarResearchRating, ProjectBrief, ResearchJob
...
class ResearchService(Layer2ResearchMixin):
    ...
    def run_job(self, job_id: str) -> None:
        ...
        try:
            ...
            elif job.job_type == "layer2_feature_competitors":
    ...
    def _run_layer1(self, job: ResearchJob) -> None:
        ...
        researched = {page.competitor_name.casefold() for page in pages}
        missing = [seed.name for seed in seeds if seed.name.casefold() not in researched]
        warnings = [f"No usable public product pages were found for {name}." for name in missing]
        self.db.update_research_job(
            job.id,
            progress=75,
            details={**job.details, "pages": len(pages), "chunks": chunks, "warnings": warnings},
        )
    ...
    def _suggest_competitors(
        self,
        brief: ProjectBrief,
        *,
        existing: set[str],
        assignment: str = "layer0_research",
    ) -> list[CompetitorSeed]:
        ...
        try:
            runtime = self._llm_runtime(brief.project_id, assignment)
